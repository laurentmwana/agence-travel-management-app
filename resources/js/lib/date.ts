...
export const formatDate = (dateString: string, lang: string = 'fr-FR') => {
...
export const parseToDateString = (dateString: string) => {
    const instance = new Date(dateString);

    return `${instance.getFullYear()}-${instance.getMonth()}-${instance.getDay()}`;
};
